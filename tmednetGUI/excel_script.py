...
df_paper.rename(columns={'Season_of_survey': 'EvenStart', 'Surveyed_site': 'Location',
                         ...
                         'Percentage_of_affected_organisms': 'Damaged percentage'}, inplace=True)
df_BA = pd.read_excel('../res/data.xlsx', sheet_name='Excel BA=YES')
merge_on = ['Year', 'EvenStart', 'Location', 'Mortality Lower Depth', 'Mortality Upper Depth', 'Species', 'Damaged percentage']
df_merge = pd.merge(df_paper[merge_on], df_BA, on=merge_on, how='inner')
df_merge = df_merge[df_BA.columns]
df_merge.to_excel('output2.xlsx', index=False)